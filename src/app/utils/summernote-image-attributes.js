...
(function (factory) {
  ...
  $.extend(true, $.summernote.lang, {
    'en-US': {
      ...
      imageAttributes: {
        dialogTitle:
          'Click on "Link / URL" to setup the site/page the user will be taken to when the image is clicked',
        ...
        linkRel: 'Rel',
        linkRelInfo:
          'Options: alternate, author, bookmark, help, license, next, nofollow, noreferrer, prefetch, prev, search, tag',
        linkRole: 'Role',
        tabUpload: 'Upload New Image',
        upload: 'Upload',
        tabBrowse: 'Browse',
        editBtn: 'OK',
      },
    ...
    imageAttributes: function (context) {
      ...
      if (options.maximumImageFileSize) {
        ...
        var readableSize =
          ...
          'B';
        imageAttributesLimitation =
          '<small class="help-block note-help-block">' +
      ...
      this.initialize = function () {
        ...
        var body =
          ...
          '  <li class="noteHidden"><a href="#note-imageAttributes' +
          timestamp +
          '" data-bs-toggle="tab">' +
          lang.imageAttributes.tabImage +
          '</a></li>' +
          '  <li class="noteHidden" ><a href="#note-imageAttributes-attributes' +
          timestamp +
          '" data-bs-toggle="tab">' +
          lang.imageAttributes.tabAttributes +
          '</a></li>' +
          '  <li class="nav-item active text-dark"><a data-bs-toggle="tab" data-bs-target="#note-imageAttributes-link' +
          timestamp +
          '">' +
          lang.imageAttributes.tabLink +
          '</a></li>';
        if (options.imageAttributes.disableUpload == false) {
          body +=
            '  <li class="nav-item text-dark"><a data-bs-toggle="tab" data-bs-target="#note-imageAttributes-upload' +
            timestamp +
            '">' +
            lang.imageAttributes.tabUpload +
            '</a></li>';
        }
        body +=
          '</ul>' +
          ...
          '  </div>' +
          // Tab 3
          '  <div class="tab-pane fade show active" id="note-imageAttributes-link' +
          timestamp +
          '">' +
          ...
          '      </div>' +
          '    </div>' +
          '    <div class="note-form-group form-group note-group-imageAttributes-link-target noteHidden">' +
          '      <label class="control-label note-form-label col-xs-3">' +
          lang.imageAttributes.linkTarget +
          '</label>' +
          '      <div class="input-group note-input-group col-xs-12 col-sm-9">' +
          '        <input class="note-imageAttributes-link-target form-control note-form-control note-input" value="_blank" type="text">' +
          '      </div>' +
          '      <small class="help-block note-help-block text-right">' +
          lang.imageAttributes.linkTargetInfo +
          '</small>' +
          '    </div>' +
          '    <div class="note-form-group form-group note-group-imageAttributes-link-class noteHidden">' +
          '      <label class="control-label note-form-label col-xs-3">' +
          ...
          '      </div>' +
          '    </div>' +
          '    <div class="note-form-group form-group note-group-imageAttributes-link-style noteHidden">' +
          '      <label class="control-label note-form-label col-xs-3">' +
          ...
          '      </div>' +
          '    </div>' +
          '    <div class="note-form-group form-group note-group-imageAttributes-link-rel noteHidden">' +
          '      <label class="control-label note-form-label col-xs-3">' +
          ...
          '</small>' +
          '    </div>' +
          '    <div class="note-form-group form-group note-group-imageAttributes-link-role noteHidden">' +
          '      <label class="control-label note-form-label col-xs-3">' +
        ...
        if (options.imageAttributes.disableUpload == false) {
          // Tab 4
          body +=
            '  <div class="tab-pane " id="note-imageAttributes-upload' +
            timestamp +
            '">' +
            '   <label class="control-label note-form-label col-xs-3">' +
        ...
        // Tab 1
        body +=
          '  <div class="tab-pane note-tab-pane fade in noteHidden" id="note-imageAttributes' +
          timestamp +
          '">' +
          ...
          '      </div>' +
          '    </div>' +
          '    <div class="note-form-group form-group note-group-imageAttributes-title active">' +
          '      <label class="control-label note-form-label col-sm-3">' +
        ...
        this.showImageAttributesDialog(imgInfo).then(function (imgInfo) {
          ...
          if (imgInfo.linkHref) {
            ...
            if (imgInfo.linkClass)
              linkBody += ' class="' + imgInfo.linkClass + '"';
            if (imgInfo.linkStyle)
              linkBody += ' style="' + imgInfo.linkStyle + '"';
            linkBody +=
              ' href="' +
              imgInfo.linkHref +
              '" target="' +
            ...
            if (imgInfo.linkRole)
              linkBody += ' role="' + imgInfo.linkRole + '"';
            linkBody += '></a>';
            $img.wrap(linkBody);
          }
      ...
      this.showImageAttributesDialog = function (imgInfo) {
        return $.Deferred(function (deferred) {
          var $imageTitle = self.$dialog.find('.note-imageAttributes-title'),
            ...
            $linkTarget = self.$dialog.find(
              '.note-imageAttributes-link-target'
            ),
